...
3. Open **VR - Risk score and rating health**, choose **Preview Update Set**, review any preview problems, and then **Commit Update Set**. The package contains seven customer updates: one Script Include, one test, and five steps (each step includes its input values).
...
The package uses stable IDs. Reimporting updates the same helper and steps, including their editable settings, so preserve your configured version before applying a later package. Step payloads use ServiceNow's SDK format to replace input values belonging only to those five steps.
...
Keep the generated class name and `type` as `VRRiskDataHealth`. The single configuration block near the top is used by all five steps.
...
| 2 | Population size | [02-population.js](steps/02-population.js) | At least `minimumRecords` matching VITs; empty data fails |
...
The helper uses database aggregates over the **whole filtered population**, without loading every VIT or silently taking a sample. Each data step performs an ungrouped count and a grouped count by score/rating. If their totals disagree, it fails rather than computing percentages from incomplete data. Each step re-reads data; this is not a transactionally frozen snapshot, and equal-count updates can still happen between queries. Schedule it when imports and scoring are idle. Assess runtime against your instance's volume and ATF limits.

Failure output contains the population, record count, and only the failed checks or bounds. A healthy distribution prints one compact line with all five percentages and the mean. Percentages use **all selected VITs** as the denominator; invalid data fails instead of being discarded. Comparison uses unrounded values; displayed values are rounded.
